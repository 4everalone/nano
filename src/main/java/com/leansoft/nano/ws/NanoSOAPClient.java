   ...
   protected void invoke(Object requestObject, SOAPServiceCallback<?> callback, Class<?> bindClazz) {
      ...
      try {
         ...
         if (soapMessage == null) {
            ...
            Map<String, String> headers = asyncHttpClient.getHeaders();
            ByteArrayEntity soapEntityByte = null;
            StringEntity soapEntityString = null;
            boolean encodeToGzip = !headers.isEmpty() && headers.containsKey(HEADER_CONTENT_ENCODING) && 
                                    headers.get(HEADER_CONTENT_ENCODING).equals(ENCODING_GZIP); 
            
            if( encodeToGzip )
            {
                ByteArrayOutputStream obj=new ByteArrayOutputStream(soapMessage.getBytes().length); 
                GZIPOutputStream gzip = new GZIPOutputStream(obj); 
                gzip.write(soapMessage.getBytes(charset));
                gzip.flush();
                gzip.close();

                soapEntityByte = new ByteArrayEntity( obj.toByteArray() );
            }
            else
            {
                soapEntityString = new StringEntity(soapMessage, charset);
            }
         ...
         asyncHttpClient.post(context, urlWithQueryString, null, (soapEntityByte != null)? soapEntityByte: soapEntityString, contentType, soapHttpResponseHandler);
